...
当前内容已覆盖美国 50 州，每个州都有总览页和 REAL ID 页；37 个跨州专题页显示专题级核对日期。州总览负责业务分流、机构入口和本州细节，REAL ID 页负责材料、流程和常见错误，避免两类页面重复堆叠同一批内容。编辑备注不会直接出现在专题页，州页和目录只使用经过公开层过滤的事实型提示。8 个高风险专题已有 44 条“重点事实与官方依据”映射，页面中的来源编号能直接对应页尾官方来源。`audit:content` 会检查更新时间、官方来源、材料/流程数量、重复入口、相关链接以及事实来源是否存在于本页来源库；`audit:seo` 会检查构建后的标题、描述、canonical、修改时间、结构化数据、sitemap 和前台内部术语。部分州政府域名会要求验证码、限制自动访问或响应较慢，相关页面已增加 USA.gov 州机动车服务目录或州级备用入口；详见 `CONTENT_AUDIT.md`。
