...
当前内容已覆盖美国 50 州，每个州都有总览页和 REAL ID 页；37 个跨州专题页显示专题级核对日期，并登记 396 条“重点事实与官方依据”映射。州总览负责业务分流、机构入口和本州细节，REAL ID 页负责材料、流程和常见错误，避免两类页面重复堆叠同一批内容。编辑备注不会直接出现在专题页，州页和目录只使用经过公开层过滤的事实型提示。费用、期限、材料、外国驾照和身份五个高风险目录只显示能够通过州级来源白名单与语义匹配的具体提示，当前构建有 372 条逐条来源映射。`audit:content` 检查内容结构和专题事实来源；`audit:directory-evidence` 阻止复合事实、编辑元话语、来源标题错配和目录覆盖退化；`audit:seo` 检查标题、描述、canonical、修改时间、结构化数据、sitemap 和前台内部术语。部分州政府域名会要求验证码、限制自动访问或响应较慢，相关页面已增加 USA.gov 州机动车服务目录或州级备用入口；详见 `CONTENT_AUDIT.md`。
