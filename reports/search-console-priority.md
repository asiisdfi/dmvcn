...
- 页面查询映射：19/38 个页面在 7 天有效期内；过期页面只暂停自身动作。
- 纳入页数：120
...
- /states/utah/（展现 128）— 先在 Search Console 中过滤该页面并采集查询，不先改正文。
- /states/georgia/（展现 93）— 先在 Search Console 中过滤该页面并采集查询，不先改正文。
- /states/north-carolina/（展现 86）— 先在 Search Console 中过滤该页面并采集查询，不先改正文。
- /states/new-jersey/（展现 135）— 先在 Search Console 中过滤该页面并采集查询，不先改正文。
- /states/texas/（展现 43）— 先在 Search Console 中过滤该页面并采集查询，不先改正文。
- /topics/older-driver-license-renewal-medical-review/（展现 52）— 先在 Search Console 中过滤该页面并采集查询，不先改正文。
- /topics/vehicle-registration-renewal-expired-tags-non-operation/（展现 484）— 先在 Search Console 中过滤该页面并采集查询，不先改正文。
- /states/massachusetts/（展现 83）— 先在 Search Console 中过滤该页面并采集查询，不先改正文。
- /states/new-york/（展现 50）— 先在 Search Console 中过滤该页面并采集查询，不先改正文。
- /states/new-york/real-id/（展现 15）— 先在 Search Console 中过滤该页面并采集查询，不先改正文。
- /topics/lost-stolen-license-plates-registration-card-sticker/（展现 353）— 先在 Search Console 中过滤该页面并采集查询，不先改正文。
- /topics/renewal-replacement-address/（展现 109）— 先在 Search Console 中过滤该页面并采集查询，不先改正文。
...
- /topics/tickets-tolls-insurance-lapse-registration-hold/（展现 83）— 2026-07-29 已设置 noindex，2026-08-26 复查 Google 索引状态；观察期内不按这些曝光扩写内容。
...
- /states/south-carolina/（展现 87）— 现有页面级信号来自泛英文或本地查询，不为非目标曝光扩写中文内容。
- /states/colorado/real-id/（展现 71）— 现有页面级信号来自泛英文或本地查询，不为非目标曝光扩写中文内容。
- /states/washington/real-id/（展现 70）— 现有页面级信号来自泛英文或本地查询，不为非目标曝光扩写中文内容。
- /states/connecticut/（展现 62）— 现有页面级信号来自泛英文或本地查询，不为非目标曝光扩写中文内容。
- /states/rhode-island/（展现 56）— 现有页面级信号来自泛英文或本地查询，不为非目标曝光扩写中文内容。
- /states/michigan/（展现 56）— 现有页面级信号来自泛英文或本地查询，不为非目标曝光扩写中文内容。
- /states/new-hampshire/（展现 54）— 现有页面级信号来自泛英文或本地查询，不为非目标曝光扩写中文内容。
- /states/north-carolina/real-id/（展现 52）— 现有页面级信号来自泛英文或本地查询，不为非目标曝光扩写中文内容。
- /states/indiana/（展现 50）— 现有页面级信号来自泛英文或本地查询，不为非目标曝光扩写中文内容。
